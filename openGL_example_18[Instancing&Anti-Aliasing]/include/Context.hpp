...
void    Context::Render(void)
{
    ...
    // FrameBuffer (Multisample Anti-Aliasing을 위해 주석)
    ...
    m_grassTexture->Bind();
    // 프레임마다 10000번의 풀을 찍어야하기 때문에 비효율적이다.
    // for (size_t idx = 0; idx < m_grassPos.size(); ++idx)
    // {
    //     modelTransform =
    //         glm::translate(glm::mat4(1.0f), glm::vec3(m_grassPos[idx].x, 0.5f, m_grassPos[idx].z)) *
    //         glm::rotate(glm::mat4(1.0f), m_grassPos[idx].y, glm::vec3(0.0f, 1.0f, 0.0f));
    //     transform = projection * view * modelTransform;
    //     m_grassProgram->SetUniform("transform", transform);
    //     m_plane->Draw(m_grassProgram.get());
    // }

    // glDrawElementsInstanced를 한번만 호출하면서 가시성, 효율성면에서 좋다.
    ...
    // Post-Processing (Multisample Anti-Aliasing할때는 주석)
